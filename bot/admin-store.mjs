...
const ADMINS_KEY = 'ids';
const BLOB_STORE = 'the4-drop';
const BLOB_ADMINS = 'the4-admins';
...
async function loadStore() {
  ...
  let store = await blob.get(STORE_KEY, { type: 'json' });
  if (!store) {
    store = defaultStore();
    store.phase = computePhase(store);
    await blob.setJSON(STORE_KEY, store);
    return store;
  }
  store = { ...defaultStore(), ...store, holds: store.holds || {}, waitlist: store.waitlist || [] };
  store.phase = computePhase(store);
  return store;
}
...
export function toSnapshot(store) {
  purgeHolds(store);
  return {
    phase: store.phase,
    stock: store.stock,
...
export async function isAdmin(telegramId) {
  ...
  const blob = getStore(BLOB_ADMINS);
  const list = (await blob.get(ADMINS_KEY, { type: 'json' })) || [];
  return list.map(String).includes(id);
}

export async function grantAdmin(telegramId) {
  const id = String(telegramId);
  const blob = getStore(BLOB_ADMINS);
  const list = (await blob.get(ADMINS_KEY, { type: 'json' })) || [];
  if (!list.map(String).includes(id)) {
    list.push(id);
    await blob.setJSON(ADMINS_KEY, list);
  }
}
